...
required_cols = [
    'store_id', 'product_id', 'sale_date', 'sale_amount', 'promo_flag',
    'stock_hour6_22_cnt', 'sale_qty', 'discount', 'original_price',
    'stock_hour6_14_cnt', 'stock_hour14_22_cnt', 'holiday_flag', 'created_at'
]
...
for _, row in df.iterrows():
    cur.execute(
        ...
        """,
        (
            int(row['store_id']), int(row['product_id']), row['sale_date'], float(row['sale_amount']), int(row['sale_qty']),
            float(row['discount']), float(row['original_price']), int(row['stock_hour6_22_cnt']), int(row['stock_hour6_14_cnt']),
            int(row['stock_hour14_22_cnt']), bool(row['holiday_flag']), bool(row['promo_flag']), row['created_at']
        )
    )
    rows_imported += 1
