...
def test_sales_transformations():
    ...
    print("Roundtrip Success:", all(abs(a - b) < 0.0001 for a, b in zip(sample_hours, encoded_hours)))
...
def test_mappings():
    ...
    for _, store in stores.iterrows():
        ...
        real_avg_sale = decode_sales_amount(store['avg_sale_amount'], store['city_id'], 0)  # Using 0 as default management group
        print(f"\nStore: {store_name}")
        print(f"Performance:")
        ...
        if store['sample_hour_sale']:
            real_hourly = decode_hourly_sales(store['sample_hour_sale'], store['city_id'], 0)
            if real_hourly:
                print("- Sample Hourly Sales (Real):", [f"¥{x:.2f}" for x in real_hourly[:5]] + ["..."])
    ...
    for _, city in weather.iterrows():
        ...
        print(f"- Price Adjustment Factor: {CITY_PRICE_ADJUSTMENTS[city['city_id']]:.2f}")
