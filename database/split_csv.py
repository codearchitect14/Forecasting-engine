...
def split_csv_file(input_file, output_dir, max_rows_per_file=500000, prefix="sales_data_part"):
    """
    ...
    print(f"Will create {num_chunks} files with approximately {max_rows_per_file} rows each")
    ...
    for i, chunk in enumerate(tqdm(pd.read_csv(input_file, chunksize=max_rows_per_file), 
                                   total=num_chunks, 
...
def main():
    ...
    # Split the file
    output_files = split_csv_file(input_file, output_dir, max_rows_per_file=optimal_rows)
