...
def load_dataset_from_huggingface(dataset_name="Dingdong-Inc/FreshRetailNet-50K", split="train", limit=None):
    """
    ...
    try:
        ...
        if split in ds:
            ...
            print(f"Split '{split}' not found in the dataset. Available splits: {list(ds.keys())}")
            if len(ds.keys()) > 0:
...
def create_sales_data_csv(df, output_file="sales_data_csv.csv"):
    ...
    sales_data['activity_flag'] = df['activity_flag'] if 'activity_flag' in df.columns else 0
...
def create_promotions_csv(df, output_file="promotion_csv.csv", sample_frac=0.05):
    ...
    store_products = promo_df[['store_id', 'product_id']].drop_duplicates().sample(frac=sample_frac, random_state=42)
    ...
    # Generate promotion data
    promotion_data = []
    for _, row in tqdm(store_products.iterrows(), total=len(store_products), desc="Generating promotions"):
        ...
        if len(sp_data) > 0:
            ...
            for _ in range(num_promos):
                ...
                promotion_data.append({
                    ...
                    'location': np.random.choice(['Entrance', 'End Cap', 'Checkout', 'Aisle', 'Special Area']),
