...
def upload_all_chunks(chunks_dir, table_name="sales_data", file_pattern="sales_data_part_*.csv"):
    """
...
def main():
    ...
    chunks_dir = os.path.join(os.path.dirname(os.path.dirname(os.path.abspath(__file__))), 
                             "data_export", 
                             "sales_data_chunks")
    ...
    table_name = input("Enter target table name (default: sales_data): ").strip() or "sales_data"
    
    # Ask for batch size
    try:
        batch_size = int(input("Enter batch size for upload (default: 1000): ").strip() or "1000")
    except ValueError:
        batch_size = 1000
        print("Invalid batch size, using default of 1000")
    
    # Ask which files to upload
    file_pattern = input("Enter file pattern to match (default: sales_data_part_*.csv): ").strip() or "sales_data_part_*.csv"
