...
def create_db_engine(connection_string=None):
    ...
    try:
        ...
        print(f"Successfully connected to database at {connection_string.split('@')[1].split('/')[0]}")
        return engine
    except Exception as e:
...
def main():
    ...
    parser = argparse.ArgumentParser(description='Load FreshRetailNet-50K dataset to PostgreSQL database')
    parser.add_argument('--dataset', type=str, default="Dingdong-Inc/FreshRetailNet-50K",
    ...
    try:
        ...
        print(f"Uploading complete dataset to database in batches of {args.batch_size}...")
